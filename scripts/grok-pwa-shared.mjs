...
function coerceHeadContext(appNameOrCtx, projectId, creator, creatorId) {
  if (typeof appNameOrCtx === "string") {
    return {
      appName: appNameOrCtx,
      projectId: projectId ?? "",
      creator: creator ?? "",
      creatorId: creatorId ?? "",
    };
  }
  return appNameOrCtx && typeof appNameOrCtx === "object" ? appNameOrCtx : {};
}

export function injectGrokPwaHead(html, appNameOrCtx = {}, projectId, creator, creatorId) {
  if (typeof html !== "string") return html;
  const ctx = coerceHeadContext(appNameOrCtx, projectId, creator, creatorId);
  const { site, projectId: pid, creator: cr, creatorId: cid, host } = normalizeHeadContext(ctx);
  ...
  // Fill missing PWA/platform tags. Do not strip or overwrite share metas the
  // document already declared (existing twitter:card, og:title, etc.).
  let next = html;
  ...
  if (!/\bname=["']twitter:card["']/i.test(next)) {
    missing.push('<meta name="twitter:card" content="summary_large_image">');
  }

  for (const tag of grokOgHeadTags({ host, appName, site, documentTitle })) {
    if (tag.includes('name="twitter:card"')) continue;
    if (tag.includes('property="og:title"')) continue;
    const prop = tag.match(/\bproperty=["']([^"']+)["']/)?.[1];
    const name = tag.match(/\bname=["']([^"']+)["']/)?.[1];
    const key = prop || name;
    if (!key) continue;
    const already =
      next.includes(`property="${key}"`) ||
      next.includes(`property='${key}'`) ||
      next.includes(`name="${key}"`) ||
      missing.some((m) => m.includes(`property="${key}"`) || m.includes(`name="${key}"`));
    if (!already) missing.push(tag);
  }
