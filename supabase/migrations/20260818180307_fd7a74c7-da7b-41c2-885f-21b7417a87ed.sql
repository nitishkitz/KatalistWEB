...
BEGIN
  BEGIN
    ...
    -- Under Progress -> reassign -> Waiting for Catch + Under Progress + pace NULL
    v_thing := public.reassign_thing(t_id, a_cee);
    results := results || format('%s|handover keeps the Thing Under Progress|ack=%s pace=%s status=%s imp=%s',
      CASE WHEN v_thing.acknowledgement='waiting_for_catch' AND v_thing.assignee_personal_pace IS NULL
             AND v_thing.work_status='under_progress' AND v_thing.owner_importance='now' THEN PASS ELSE 'FAIL' END,
      v_thing.acknowledgement, v_thing.assignee_personal_pace, v_thing.work_status, v_thing.owner_importance);
    results := results || format('%s|handover clears caught_at|caught_at=%s',
      CASE WHEN v_thing.caught_at IS NULL THEN PASS ELSE 'FAIL' END, v_thing.caught_at);
    ...
    -- the new holder must Catch before touching Work Status
    PERFORM set_config('request.jwt.claims', json_build_object('sub', cee_id, 'role','authenticated')::text, true);
    BEGIN
      PERFORM public.set_work_status(t_id, 'under_progress');
      results := results || 'FAIL|new holder cannot change Work Status before Catch|no error raised';
    EXCEPTION WHEN OTHERS THEN
      results := results || format('%s|new holder cannot change Work Status before Catch|%s', PASS, SQLERRM);
    END;
    BEGIN
      PERFORM public.sort_thing(t_id);
      results := results || 'FAIL|new holder cannot Sort before Catch|no error raised';
    EXCEPTION WHEN OTHERS THEN
      results := results || format('%s|new holder cannot Sort before Catch|%s', PASS, SQLERRM);
    END;
    BEGIN
      PERFORM public.set_personal_pace(t_id, 'now');
      results := results || 'FAIL|new holder cannot set Pace before Catch|no error raised';
    EXCEPTION WHEN OTHERS THEN
      results := results || format('%s|new holder cannot set Pace before Catch|%s', PASS, SQLERRM);
    END;

    v_thing := public.catch_thing(t_id);
    results := results || format('%s|new holder Catch gives pace NEXT and keeps Under Progress|pace=%s status=%s ack=%s',
      CASE WHEN v_thing.assignee_personal_pace='next' AND v_thing.work_status='under_progress'
             AND v_thing.acknowledgement='caught' THEN PASS ELSE 'FAIL' END,
      v_thing.assignee_personal_pace, v_thing.work_status, v_thing.acknowledgement);
    ...
    SELECT count(*) INTO v_cnt FROM public.thing_activity ta
     WHERE ta.thing_id = t_id AND ta.event = 'reassigned'
       ...
       AND ta.detail ->> 'work_status_preserved' = 'under_progress'
       AND (ta.detail ->> 'from_assignment_id') IS NOT NULL
       AND (ta.detail ->> 'to_assignment_id')   IS NOT NULL;
    results := results || format('%s|handover history records real actors and preserved status|matches=%s',
      CASE WHEN v_cnt=1 THEN PASS ELSE 'FAIL' END, v_cnt);

    -- a person who hands a Thing on keeps nothing; the Thing Owner keeps everything
    PERFORM set_config('request.jwt.claims', json_build_object('sub', cee_id, 'role','authenticated')::text, true);
    ...
    results := results || format('%s|person who handed the Thing on loses access|rows=%s',
      CASE WHEN v_cnt=0 THEN PASS ELSE 'FAIL' END, v_cnt);
    PERFORM set_config('request.jwt.claims', json_build_object('sub', cee_id, 'role','authenticated')::text, true);
    SELECT count(*) INTO v_cnt FROM public.things WHERE id = h_id;
    results := results || format('%s|Thing Owner keeps access after handover|rows=%s',
      CASE WHEN v_cnt=1 THEN PASS ELSE 'FAIL' END, v_cnt);
    PERFORM set_config('request.jwt.claims', json_build_object('sub', bee_id, 'role','authenticated')::text, true);
    SELECT count(*) INTO v_cnt FROM public.things WHERE id = h_id;
    results := results || format('%s|uninvolved person cannot see a handed-off Thing|rows=%s',
      CASE WHEN v_cnt=0 THEN PASS ELSE 'FAIL' END, v_cnt);
...
-- service-role-only entry points for the Bridge end-to-end test script
CREATE OR REPLACE FUNCTION public.test_bridge_fixture()
...
LANGUAGE sql
SET search_path TO 'pg_catalog', 'public', 'katalist_priv'
AS $$ SELECT * FROM katalist_priv.test_bridge_fixture(); $$;

CREATE OR REPLACE FUNCTION public.test_bridge_owner(
  p_profile uuid, p_action text, p_thing_id uuid, p_target uuid DEFAULT NULL)
RETURNS text
LANGUAGE sql
SET search_path TO 'pg_catalog', 'public', 'katalist_priv'
AS $$ SELECT katalist_priv.test_bridge_owner(p_profile, p_action, p_thing_id, p_target); $$;

CREATE OR REPLACE FUNCTION public.test_bridge_state(p_thing_id uuid)
RETURNS jsonb
LANGUAGE sql
SET search_path TO 'pg_catalog', 'public', 'katalist_priv'
AS $$ SELECT katalist_priv.test_bridge_state(p_thing_id); $$;

CREATE OR REPLACE FUNCTION public.test_bridge_cleanup()
RETURNS void
LANGUAGE sql
SET search_path TO 'pg_catalog', 'public', 'katalist_priv'
AS $$ SELECT katalist_priv.test_bridge_cleanup(); $$;

REVOKE ALL ON FUNCTION public.test_bridge_fixture()                       FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.test_bridge_owner(uuid, text, uuid, uuid)   FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.test_bridge_state(uuid)                     FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.test_bridge_cleanup()                       FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.test_bridge_fixture()                     TO service_role;
GRANT EXECUTE ON FUNCTION public.test_bridge_owner(uuid, text, uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.test_bridge_state(uuid)                   TO service_role;
GRANT EXECUTE ON FUNCTION public.test_bridge_cleanup()                     TO service_role;

COMMENT ON FUNCTION public.test_bridge_fixture() IS 'TEST INFRASTRUCTURE — service_role only.';
COMMENT ON FUNCTION public.test_bridge_owner(uuid, text, uuid, uuid) IS 'TEST INFRASTRUCTURE — service_role only.';
COMMENT ON FUNCTION public.test_bridge_state(uuid) IS 'TEST INFRASTRUCTURE — service_role only.';
COMMENT ON FUNCTION public.test_bridge_cleanup() IS 'TEST INFRASTRUCTURE — service_role only.';