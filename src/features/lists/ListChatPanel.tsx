...
export function ListChatPanel({ chat, message, onMessage }: { chat: ReturnTypeUseListMessages; message: string; onMessage: (value: string) => void }) {
  ...
  return (
    <section className="flex h-[min(65vh,38rem)] min-h-[24rem] flex-col overflow-hidden rounded-xl border border-border bg-card">
      <p className="shrink-0 border-b border-border p-4 text-[12px] text-muted-foreground">List Chat is shared room conversation. Thing comments stay on each Thing.</p>
      ...
      <form className="flex shrink-0 gap-2 border-t border-border bg-card p-3" onSubmit={(event) => { event.preventDefault(); if (!message.trim() || chat.send.isPending) return; void chat.send.mutateAsync(message.trim()).then(() => onMessage(""), (error) => toast.error(domainErrorMessage(error))); }}>
