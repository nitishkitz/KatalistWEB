...
export function ThingDetailContent({
  ...
  return (
    <div className="min-h-full bg-white" data-detail-layout="compact">
      <header
        className="sticky top-0 z-10 space-y-1 border-b border-border/70 bg-white px-5 py-3 text-left"
        data-detail-region="header"
      >
        <div className="flex items-start justify-between gap-3">
          <h2 className="min-w-0 text-[17px] font-semibold leading-snug text-foreground">
            {thing.title}
          </h2>
          {headerAction}
        </div>
        <p className="flex flex-wrap items-center gap-1.5 text-[10.5px] text-muted-foreground">
          <span className="font-medium uppercase tracking-wide">{thing.context}</span>
          {thing.listName ? (
            <>
      ...
      <div className="space-y-3 px-5 py-3">
        ...
        <section className="space-y-1.5" data-detail-region="people">
          <h3 className="katalist-section-title">People</h3>
          <div className="grid gap-2 rounded-lg border border-border/70 bg-white p-2.5 sm:grid-cols-[1fr_auto_1fr]">
            <div className="flex min-h-8 flex-col items-start justify-center gap-0.5">
              <span className="text-[11px] text-muted-foreground">Creator</span>
              <PersonCell person={thing.creator} />
            </div>
            <span
              className="hidden items-center justify-center text-lg text-muted-foreground sm:flex"
              aria-hidden="true"
            >
              →
            </span>
            ...
            <div className="flex min-h-7 items-center gap-2 border-t border-border/70 pt-2 sm:col-span-3">
              <span className="text-[11px] text-muted-foreground">Owner</span>
              <PersonCell person={thing.owner} />
            </div>
          </div>
          <label className="flex h-8 items-center gap-2 px-1 text-[11px] text-muted-foreground">
        ...
        {caps?.canAddToBucket ? (
          <section className="space-y-1.5" data-detail-region="metadata">
            <h3 className="katalist-section-title">Add to Bucket</h3>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
                  className="flex h-9 w-full items-center gap-2 rounded-lg border border-border bg-white px-3 text-[11px] outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
              ...
              <DropdownMenuContent
                align="start"
                className="w-[320px] max-w-[calc(100vw-4rem)] bg-white"
              >
                <DropdownMenuLabel className="text-[11px] text-muted-foreground">
                  Add to one or more Buckets
                </DropdownMenuLabel>
                {buckets.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <section className="space-y-1.5" data-detail-region="controls">
            ...
            {caps?.canCatch ? (
              <CatchActionButton
                thing={thing}
                className="inline-flex h-8 items-center justify-center gap-2 rounded-lg border border-primary bg-white px-3 text-[11px] font-medium text-primary hover:bg-primary/5"
              >
          ...
          <section className="space-y-1.5" data-detail-region="pace">
            ...
            <div className="grid grid-cols-3 overflow-hidden rounded-lg border border-border bg-white">
              {paces.map((p) => (
                <button
                  ...
                  className={cn(
                    "h-8 border-r border-border text-[10px] font-semibold uppercase outline-none last:border-r-0 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring",
                    paceTone[p],
                    activePace === p ? "bg-muted/60" : "bg-white hover:bg-muted/30",
            ...
            <div className="sr-only" aria-live="polite">
              My pace: {activePace.toUpperCase()}
            </div>
          </section>

          <section className="space-y-1.5" data-detail-region="status">
            <div className="flex items-center justify-between">
              ...
              {statuses.map((s) => (
                <button
                  ...
                  className={cn(
                    ...
                    thing.workStatus === s
                      ? s === "under_progress"
                        ? "border-status-next/30 bg-status-next/10 text-status-next"
                        : s === "sorted"
                          ? "border-status-caught/30 bg-status-caught/10 text-status-caught"
        ...
        <div
          className="grid grid-cols-2 gap-2 border-t border-border/70 pt-3"
      ...
      <div
        className="border-t border-border/70 bg-white px-5 py-3"
        data-detail-region="conversation"
      >
