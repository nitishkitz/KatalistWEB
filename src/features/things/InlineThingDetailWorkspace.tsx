...
function ListThingNavigator({
  ...
  return (
    <nav
      className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-border/80 bg-white p-3 shadow-xs"
      aria-label={`${title} Navigator`}
    >
      <div className="mb-2.5 flex items-start justify-between px-1">
        <div>
          <div className="flex items-center gap-1.5">
            <FileText className="h-4 w-4 text-primary" />
            <h2 className="text-[12.5px] font-bold tracking-[0.05em] text-foreground">
              {title}
            </h2>
          </div>
          <p className="mt-0.5 text-[10.5px] text-muted-foreground">Click to inspect or edit</p>
        </div>
  ...
  return (
    <section
      ...
      className={cn(
        ...
        hasNavigator
          ? "lg:grid-cols-[minmax(240px,290px)_minmax(480px,1fr)]"
