      ...
      <div className="flex -space-x-1.5">
        {shown.map((m) => (
          <span
            ...
            className="flex h-6 w-6 items-center justify-center rounded-full border-2 border-card bg-muted text-[9px] font-semibold text-foreground"
            title={m.name}
          >
            {m.initials}
          </span>
          ...
          {rows.map((row) => (
            <tr key={row.id} className="cursor-pointer border-t border-border/80 hover:bg-muted/40">
              ...
              <td className="pr-3 text-right">
                <button type="button" className="rounded p-1 text-muted-foreground hover:bg-muted" aria-label="List actions">
